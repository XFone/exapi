...
RestRequest::CreateBuilder(Uri &uri) {
    //std::string _uri = uri.protoclstr + uri.domain + uri.api;
    //LOGFILE(LOG_DEBUG, "URI: %s", _uri.c_str());

    //auto req = std::make_shared<RestRequest>(restbed::Uri("https://www.okcoin.com/api/v1/ticker.do?symbol=eth_btc"));
    auto req = std::make_shared<RestRequest>(/*restbed::Uri(_uri)*/);

    req->set_host(uri.domain);
    req->set_protocol((uri.protocol == HTTP_PROTOCOL_HTTP) ? "HTTP" : "HTTPS");
    req->set_port((uri.protocol == HTTP_PROTOCOL_HTTPS) ? 443 : 80);
    //req->set_version(1.1);
    req->set_path(uri.api);
    req->set_method((uri.method == METHOD_GET) ? "GET" : "POST");
...
RestRequest::SendSync(std::shared_ptr<RestRequest> &req) {
    ...
    try {
        ...
        // auto buf = req->m_pimpl->m_buffer.get();
