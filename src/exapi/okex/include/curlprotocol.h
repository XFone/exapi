 ...
 * OKCoinApi C++
 * 
 * Copy from https://github.com/OKCoin/rest/blob/master/C%2B%2B/net/http/curl/curlprotocol.h
 * 
 * HTTP REST path definition (www.okcoin.cn and www.okcoin.com)
 * 
 * $Log: $
 */
...
public:
    ...
    Uri &GetUrl(UINT http_protocl_type);

    map<UINT,Uri> m_urllist;

    const char *domain;
    int AddApi(UINT http_protocl_type,HTTP_PROTOCOL http_protocol,const char *api,HTTP_METHOD http_method);
