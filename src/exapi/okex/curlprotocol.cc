 ...
 * $Id: $
 *
 * CUrlProtocol implementation with C++11 lambda and ASIO features
 *  
...
static const char *_url_domain_cn  = URL_DOMAIN_CN;
static const char *_url_domain_com = URL_DOMAIN_COM;
...
{
    auto it = m_urllist.find(http_protocl_type);
    if (it != m_urllist.end()) {
        uri = it->second;
    }
    return;
...
Uri &CUrlProtocol::GetUrl(UINT http_protocl_type)
{
    auto it = m_urllist.find(http_protocl_type);
    if (it != m_urllist.end()) {
        return it->second;
    }
    return m_urllist.begin()->second; // TO CHECK
}


int CUrlProtocol::AddApi(UINT http_protocl_type, HTTP_PROTOCOL http_protocol, const char *api, HTTP_METHOD http_method)
{
    string str;

    Uri uri = {
        .type       = http_protocl_type,
